...
#include <bml/type_traits/remove_volatile.hpp>
#include <bml/type_traits/is_same.hpp>

template <typename T, typename U>
auto check_result() noexcept -> void
{
    static_assert(bml::is_same_v<typename bml::remove_volatile<T>::type, U>);
    static_assert(bml::is_same_v<bml::remove_volatile_ty<T>, U>);
}
...
auto test_main() noexcept -> int
{
    ...
    // Check that the result is the same as the const-qualified version of the input when the
    // input is const-volatile-qualified.
    {
        check_result<void const volatile, void const>();
        check_result<int const volatile, int const>();
        check_result<int const volatile[3], int const[3]>();
        check_result<int* const volatile, int* const>();
        check_result<double volatile* const volatile, double volatile* const>();
        check_result<bmltb::class_type const volatile, bmltb::class_type const>();
        check_result<bmltb::enum_class const volatile, bmltb::enum_class const>();
        check_result<bmltb::union_type const volatile, bmltb::union_type const>();
        check_result<bmltb::incomplete_class const volatile, bmltb::incomplete_class const>();
        check_result<int bmltb::class_type::* const volatile, int bmltb::class_type::* const>();
    }
