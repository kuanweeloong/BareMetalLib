...
#include <bml/type_traits/add_pointer.hpp>
#include <bml/type_traits/is_same.hpp>

template <typename T, typename U>
auto check_result() noexcept -> void
{
    static_assert(bml::is_same_v<typename bml::add_pointer<T>::type, U>);
    static_assert(bml::is_same_v<bml::add_pointer_ty<T>, U>);
}

auto test_main() noexcept -> int
{
    // Check that the result names a pointer to the input type when the input type is void.
    {
    ...
    // Check that the result names a pointer to the input type when the input type is a
    // referenceable, non-reference type.
    {
    ...
    // Check that the result names a pointer to the input's referred to type when the input is a
    // reference type.
    {
        check_result<int&, int*>();
        check_result<int const volatile&, int const volatile*>();
    ...
    // Check that the result names the same type as the input type when the input type is not
    // referenceable and not void.
    {
        check_result<auto () & -> void, auto () & -> void>();
