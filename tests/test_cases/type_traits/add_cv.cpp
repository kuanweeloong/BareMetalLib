...
#include <bml/type_traits/add_cv.hpp>
#include <bml/type_traits/is_same.hpp>

template <typename T, typename U>
auto check_result() noexcept -> void
{
    static_assert(bml::is_same_v<typename bml::add_cv<T>::type, U>);
    static_assert(bml::is_same_v<bml::add_cv_ty<T>, U>);
}

auto test_main() noexcept -> int
{
    // Check that the result is the same as the const-volatile-qualified version of the input when
    // the input is unqualified.
    {
        check_result<void, void const volatile>();
        check_result<int, int const volatile>();
        check_result<int[3], int const volatile[3]>();
        check_result<int*, int* const volatile>();
        check_result<double const*, double const* const volatile>();
        check_result<bmltb::class_type, bmltb::class_type const volatile>();
        check_result<bmltb::enum_class, bmltb::enum_class const volatile>();
        check_result<bmltb::union_type, bmltb::union_type const volatile>();
        check_result<bmltb::incomplete_class, bmltb::incomplete_class const volatile>();
        check_result<int bmltb::class_type::*, int bmltb::class_type::* const volatile>();
    }
    ...
    // the input is volatile-qualified.
    {
        ...
        check_result<int* volatile, int* const volatile>();
        check_result<double const* volatile, double const* const volatile>();
    ...
    // Check that the result is the same as the input when the input cannot be
