...
{
    //
    // A variadic version of std::min.
    // 
    // This is different from std::min in that it does not require std::initializer_list to compare
    // more than 2 arguments. In addition, this has an overload to handle a single argument (it just
    // returns a reference to that argument) - the intent is to allow min to be invoked on parameter
